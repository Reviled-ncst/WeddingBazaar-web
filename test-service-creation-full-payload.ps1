# ✅ COMPREHENSIVE TEST: Simulate exact frontend payload structure
...
try {
    ...
    if ($response.service.package_items) {
        ...
        foreach ($item in $items) {
            Write-Host "   - $($item.name): ₱$($item.unit_price) x $($item.quantity) = ₱$($item.amount)" -ForegroundColor White
        }
    }
    ...
    try {
        ...
        try {
            ...
            if ($errorJson.error) {
                Write-Host ""
                Write-Host "⚠️ Error Message: $($errorJson.error)" -ForegroundColor Red
            }
