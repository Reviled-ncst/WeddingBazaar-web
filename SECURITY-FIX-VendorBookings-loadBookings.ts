// SECURITY-FIXED VERSION OF loadBookings function for VendorBookings.tsx
// This version removes the security vulnerability and only loads bookings for the authenticated vendor

// Replace the existing loadBookings function with this secure version:
  ...
  try {
    ...
    if (directData.success && directData.bookings) {
      ...
      if (secureBookings.length > 0) {
        ...
        // Set empty bookings instead of fallback
        setBookings([]);
        setPagination({
          current_page: 1,
          total_pages: 1,
    ...
    // SECURITY: Set empty bookings array - no mock data or fallback in production
    setBookings([]);
    setPagination({
      current_page: 1,
      total_pages: 1,
