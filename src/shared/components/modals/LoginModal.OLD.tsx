...
export const LoginModal: React.FC<LoginModalProps> = ({ 
  ...
  const errorRef = useRef(false); // Track error state across renders
  ...
  useEffect(() => {
    console.log('� [LoginModal] Parent isOpen changed to:', isOpen, '| errorRef.current:', errorRef.current);
    
    // CRITICAL: Check error FIRST before any state changes
    if (errorRef.current) {
      // There's an error - LOCK modal open regardless of parent's isOpen
      console.log('🛑 [LoginModal] BLOCKING - errorRef.current is true, keeping modal open');
    ...
    if (isOpen) {
      ...
      setInternalIsOpen(true);
      // Clear states when opening (but DON'T clear errorRef here - only clear on successful close)
      setError(null);
      setIsLoginSuccess(false);
      setIsLoading(false);
    } else if (!isLoginSuccess) {
      // Parent wants to close AND no error and not showing success - allow
      console.log('✅ [LoginModal] Closing modal (no error)');
      setInternalIsOpen(false);
      errorRef.current = false; // NOW we can clear the ref on successful close
    } else {
  ...
  const handleDismissError = () => {
    ...
    setError(null);
    errorRef.current = false; // Clear the ref
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      errorRef.current = true; // Set ref FIRST
      setError(errorMessage);
      setIsLoading(false);
      setIsLoginSuccess(false);
      
      console.log('✅ [LoginModal] Error state set, errorRef.current:', errorRef.current);
