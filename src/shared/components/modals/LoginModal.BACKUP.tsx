...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  // UI state - INTERNAL MODAL STATE (not controlled by parent)
  ...
  // Reset form when modal actually closes
  React.useEffect(() => {
    if (!internalOpen) {
      setEmail('');
      setPassword('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('📝 [LoginModal.CLEAN] Setting error:', errorMessage);
      setError(errorMessage);
      setIsSubmitting(false);
      console.log('🔍 [LoginModal.CLEAN] Error state updated, modal MUST stay open');
      // DO NOT CLOSE MODAL - error prevents close
    }
  };

  // Handle modal close - BULLETPROOF LOCK
  const handleModalClose = () => {
    console.log('🚪 [LoginModal.CLEAN] Close requested | error:', error, '| isSubmitting:', isSubmitting);
    
    if (error) {
      console.log('🚫 [LoginModal.CLEAN] Cannot close - error present:', error);
      return; // STOP - DO NOT CLOSE
    }
