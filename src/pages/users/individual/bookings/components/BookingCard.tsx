...
export const BookingCard: React.FC<BookingCardProps> = ({
  ...
  if (viewMode === 'list') {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-pink-200 hover:shadow-md hover:border-pink-300 transition-all duration-200">
        <div className="p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4 flex-1">
              ...
              <div className="flex items-center space-x-2">
                ...
                <div className="relative">
                  ...
                  />
                  <div className={cn(
                    "absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center border border-white text-xs",
                    config.color.includes('green') ? 'bg-pink-500 text-white' :
                    config.color.includes('blue') ? 'bg-pink-500 text-white' :
                    config.color.includes('yellow') ? 'bg-pink-400 text-white' :
                    config.color.includes('red') ? 'bg-rose-500 text-white' :
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600 mb-1">
                  {booking.vendorBusinessName || booking.vendorName} • {booking.serviceType}
                </p>
            ...
            <div className="flex items-center space-x-2 ml-6">
              <button
                ...
                className="px-4 py-2 bg-pink-50 text-pink-700 rounded-lg hover:bg-pink-100 transition-colors duration-200 flex items-center gap-2 text-sm font-medium border border-pink-200"
              >
              ...
              {booking.eventCoordinates && onViewLocation && (
                <button
                  ...
                  className="px-4 py-2 bg-pink-100 text-pink-700 rounded-lg hover:bg-pink-200 transition-colors duration-200 flex items-center gap-2 text-sm font-medium border border-pink-300"
                >
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-pink-200 overflow-hidden hover:shadow-xl hover:border-pink-300 transition-all duration-300 hover:scale-[1.02]">
      {/* Enhanced Header with Service Image Background and Vendor Overlay */}
      <div className="relative">
        <div className="h-32 relative overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-pink-900/70 via-pink-900/20 to-transparent"></div>
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <div className="flex items-end justify-between">
              <div className="flex-1">
                ...
                <p className="text-sm text-white/90">{booking.vendorBusinessName || booking.vendorName}</p>
                <p className="text-xs text-white/80">{booking.vendorCategory || booking.serviceType}</p>
              </div>
              
              {/* Vendor Image as Small Overlay */}
              <div className="flex flex-col items-end gap-2">
                {booking.vendorRating && (
                  <div className="flex items-center space-x-1 bg-pink-500/20 backdrop-blur-sm rounded-full px-2 py-1 border border-pink-300/50">
                    <Star className="w-4 h-4 text-pink-200 fill-current" />
                ...
                <VendorImage
                  ...
                  fallbackText={booking.vendorBusinessName || booking.vendorName}
                  size="sm"
                  className="border-2 border-pink-200/50 shadow-lg"
                />
              </div>
            </div>
          </div>
      ...
      <div className="p-6">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            {booking.serviceGallery && booking.serviceGallery.length > 1 && (
              <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                {booking.serviceGallery.length} photos
              </span>
            )}
          </div>
          <p className="text-sm text-gray-600 mb-1">
          ...
          <p className="text-sm text-gray-500">Reference: {booking.bookingReference || booking.id}</p>
        </div>
