...
// Use same connection method as backend
...
async function createReviewsTable() {
  try {
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_reviews_vendor_id ON reviews(vendor_id)`;
    ...
    // 3. Insert 17 sample reviews for SRV-0001
    ...
    for (let i = 0; i < reviews.length; i++) {
      ...
      const helpfulCount = Math.floor(Math.random() * 15);
    ...
    const avgRating = parseFloat(ratingCalc.avg_rating).toFixed(1);
    const totalReviews = parseInt(ratingCalc.total_reviews);
    ...
    await sql`
      UPDATE vendors 
      SET rating = ${avgRating}, review_count = ${totalReviews}, updated_at = NOW()
      WHERE id = '2-2025-001'
    `;
    ...
    console.log('');
    console.log('🚀 Next step: Reviews API is already deployed, just needs backend restart');
