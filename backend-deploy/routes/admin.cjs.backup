...
router.get('/dashboard/stats', async (req, res) => {
  try {
    ...
    const [
      ...
      sql`SELECT COUNT(*) as count FROM users WHERE role IN ('individual', 'vendor')`,
      
      // Total vendors (distinct users with vendor role)
      sql`SELECT COUNT(DISTINCT user_id) as count FROM vendor_profiles WHERE user_id IS NOT NULL`,
