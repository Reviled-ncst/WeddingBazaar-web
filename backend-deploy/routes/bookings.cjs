  ...
  try {
    ...
    const booking = await sql`
      INSERT INTO bookings (
        ...
        package_id, package_name, package_price, package_items, selected_addons, 
...
router.post('/:bookingId/request-cancellation', async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ [REQUEST-CANCELLATION] Authorization passed: "${bookingUserId}" === "${requestUserId}"`);
