...
router.post('/', async (req, res) => {
  try {
    ...
    // ✅ FIX: Use UUID instead of sequential ID to avoid race conditions
    // PostgreSQL will auto-generate a unique UUID via gen_random_uuid()
    // We don't need to specify an ID - let the database handle it
    console.log('🆔 Service ID will be auto-generated by PostgreSQL (UUID)');
    ...
    // ✅ ID auto-generated by PostgreSQL (gen_random_uuid()) - no manual ID needed
    ...
    const serviceId = result[0].id; // ✅ Get the auto-generated UUID from the database

    console.log('✅ [POST /api/services] Service created successfully:', result[0]);
    console.log('🆔 [AUTO-GENERATED] Service ID:', serviceId);
